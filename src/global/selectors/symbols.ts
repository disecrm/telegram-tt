...
export function selectIsStickerFavorite<T extends GlobalState>(
  global: T,
  sticker: ApiSticker,
) {
...
export function selectStickerSet<T extends GlobalState>(
  global: T,
  id: string | ApiStickerSetInfo,
) {
  if (typeof id === 'string') {
  ...
  return Object.values(global.stickers.setsById).find(
    ({ shortName }) => shortName.toLowerCase() === id.shortName.toLowerCase(),
  );
}

export function selectStickersForEmoji<T extends GlobalState>(
  global: T,
  emoji: string,
) {
  ...
  addedSets?.forEach((id) => {
    ...
    stickersForEmoji = stickersForEmoji.concat(
      packs[emoji] || [],
      packs[cleanEmoji(emoji)] || [],
    );
  });
  return stickersForEmoji;
}

export function selectCustomEmojiForEmoji<T extends GlobalState>(
  global: T,
  emoji: string,
) {
  ...
  addedCustomSets?.forEach((id) => {
    ...
    customEmojiForEmoji = customEmojiForEmoji.concat(
      packs[emoji] || [],
      packs[cleanEmoji(emoji)] || [],
    );
  });
  return isCurrentUserPremium
    ? customEmojiForEmoji
    : customEmojiForEmoji.filter(({ isFree }) => isFree);
}

// Slow, not to be used in `withGlobal`
export function selectCustomEmojiForEmojis<T extends GlobalState>(
  global: T,
  emojis: string[],
) {
  const isCurrentUserPremium = selectIsCurrentUserPremium(global);
...
export function selectIsSetPremium(
...
export function selectAnimatedEmoji<T extends GlobalState>(
  global: T,
  emoji: string,
) {
  ...
  return animatedEmojis.stickers.find(
    (sticker) => sticker.emoji === emoji || sticker.emoji === cleanedEmoji,
  );
}

export function selectRestrictedEmoji<T extends GlobalState>(
  global: T,
  emoji: string,
) {
...
export function selectAnimatedEmojiEffect<T extends GlobalState>(
  global: T,
  emoji: string,
) {
...
export function selectAnimatedEmojiSound<T extends GlobalState>(
  global: T,
  emoji: string,
) {
...
export function selectIsAlwaysHighPriorityEmoji<T extends GlobalState>(
  global: T,
  stickerSet: ApiStickerSetInfo | ApiStickerSet,
) {
...
export function selectGiftStickerForStars<T extends GlobalState>(
  global: T,
  starCount?: number,
) {
  const stickers = global.premiumGifts?.stickers;
  ...
  let emoji: string; // Add explicit type annotation
  if (starCount <= 1000) {
